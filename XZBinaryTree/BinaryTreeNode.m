 ...
 */
+(void)preOrderTraverseTree:(BinaryTreeNode *)rootNode handler:(void (^)(BinaryTreeNode * _Nonnull))handler
{
    if(rootNode){
        if(handler){
            handler(rootNode);
 ...
 *  @param handler  访问节点处理函数
 */
+(void)inOrderTraverseTree:(BinaryTreeNode *)rootNode handler:(void (^)(BinaryTreeNode * _Nonnull))handler
{
    if(rootNode){
 ...
 *  @param handler  访问节点处理函数
 */
+(void)postOrderTraverseTree:(BinaryTreeNode *)rootNode handler:(void (^)(BinaryTreeNode * _Nonnull))handler
{
    if(rootNode){
 ...
 *  @param handler  访问节点处理函数
 */
+(void)levelTraverseTree:(BinaryTreeNode *)rootNode handler:(void (^)(BinaryTreeNode * _Nonnull))handler
{
    if(!rootNode){
        return;
    }
...
+(BinaryTreeNode *)invertBinaryTreeNot:(BinaryTreeNode *)rootNode
{
    ...
    while (queueArray.count>0) {
        BinaryTreeNode * node = [queueArray firstObject];
        [queueArray removeObjectAtIndex:0];//弹出最前面的节点，仿照队列先进先出
...
+ (BOOL)isFullBinaryTree:(BinaryTreeNode *)rootNode
{
    ...
    //二叉树深度
    NSInteger depth = [BinaryTreeNode depthOfTree:rootNode];
    //二叉树叶子节点数
...
+ (BOOL)isAVLBinaryTree:(BinaryTreeNode *)rootNode
{
    ...
    BOOL isAVLLeft = [BinaryTreeNode isAVLBinaryTree:rootNode.leftNode];
    NSInteger heightLeft = height;
    BOOL isAVLRight = [BinaryTreeNode isAVLBinaryTree:rootNode.rightNode];
